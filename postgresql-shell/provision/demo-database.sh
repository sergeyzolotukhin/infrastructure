...
if [ -d "/var/lib/postgresql/14/demo_tablespace" ];
then
  sudo -u postgres rm -Rf /var/lib/postgresql/14/demo_tablespace;
fi

if [ -d "/var/lib/postgresql/14/demo_tablespace_01" ];
then
  sudo -u postgres rm -Rf /var/lib/postgresql/14/demo_tablespace_01;
fi
...
sudo -u postgres psql -c "CREATE TABLESPACE demo_tablespace    OWNER postgres LOCATION '/var/lib/postgresql/14/demo_tablespace'"
sudo -u postgres psql -c "CREATE TABLESPACE demo_tablespace_01 OWNER postgres LOCATION '/var/lib/postgresql/14/demo_tablespace_01'"
